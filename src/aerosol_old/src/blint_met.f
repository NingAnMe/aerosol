...
C
C
      subroutine blint_met(grid, i, j, lat, lon, t, ret)
...
c
c!Description:
c   Bi-linear interpolation scheme used with GDAS gridded products.
c
c!Input Parameters:
...
c ... Parameters
      ...
      DIMENSION grid(0:npoints_x-1, 0:npoints_y)

c ... Local variables

      REAL p, ip0, ip1, jp0, jp1, t00, t01, t0,
     +     t10, t11, t1, ldi, ldj, colat
      INTEGER i1, j1 
      LOGICAL y_interp

      ret = 0

      if( j .lt. 0 .or. j .gt. (npoints_y) ) then
        ret = -1
      end if
      ...
      if( ret .eq. 0 ) then
        ...
        if(ldj .ge. 0.0) then
          j1 = j + 1
          if( j .eq. npoints_y ) y_interp = .false.
        else
          j1 = j - 1
