    ...
    def __get_geo_file(self):
        ...
        if self.resolution == 1000:
            ...
            if self.satellite in satellite_type1:
                ...
                pat = '\w{5}.%s.%s.\d{3}.\d+.hdf$' % (part1, part2)
...
if __name__ == '__main__':
    L1File = r'C:\E\Projects\IMAPP\IMAPP_MODISL2_INPUT_OUTPUT\test_input\MOD021KM.A2011302.1935.061.2017327062101.hdf'
...
#     print modis.file_attr  # L1 文件属性
    ...
    def date2dsol(date):
        jjj = int(date.strftime('%j'))
        OM = (0.9856 * (jjj - 4)) * np.pi / 180.
        dsol = 1. / ((1. - 0.01673 * np.cos(OM)) ** 2)
        return dsol


    def sun_earth_distance_correct_ref(ref, dsol, sz):
        """
        REF要做太阳天顶角的日地距离修整
        """
        ref_correct = ref * np.sqrt(dsol) / np.cos(np.deg2rad(sz))
        return ref_correct
...
#     print_channel_data(t_data)
    ...
    sz = modis.get_sensor_zenith()
    dsol = ymd2dsol(modis.ymd)
    print('ref:')
    t_data = modis.get_ref()
    for key in sorted(t_data.keys()):
        data = t_data[key]
        print("%s, %0.6f %0.6f" % (key, np.nanmin(data), np.nanmax(data)))
        data = sun_earth_distance_correct_ref(data, dsol, sz)
        print("%s, %0.6f %0.6f" % (key, np.nanmin(data), np.nanmax(data)))
